...
export default function SearchMovieViewWrapper({ movieInfo, viewMode }) {
  const [isActive, setActivity] = useState(
    typeof getFavorite("favoriteMovies")[movieInfo.id] === "undefined"
  );
  const { t } = useTranslation("searchPage");
  const buttonText = viewMode === "block" ? t("add") : t("addToFavorite");
  const handleClickAddButton = (e) => {
    addFavoriteMovie(e.target.id);
    setActivity(!isActive);
  };
  return (
    <MovieView movieInfo={movieInfo} viewMode={viewMode} isActive={isActive}>
      <AddFilmButton
        disabled={!isActive}
        lng={i18n.language}
